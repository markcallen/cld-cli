...
`,
  ...
  async run(): Promise<void> {
    ...
    const {AWS_PROFILE: awsProfile, AWS_ACCESS_KEY_ID: awsAccessKeyId, AWS_SECRET_ACCESS_KEY: awsSecretAccessKey} = process.env
    if (awsAccessKeyId) {
      this.log(`AWS_ACCESS_KEY_ID=${replaceWithAstrisk(awsAccessKeyId)}`)
      this.log(`AWS_SECRET_ACCESS_KEY=${replaceWithAstrisk(awsSecretAccessKey)}`)
    } else if (awsProfile) {
